	...
	export default {
		...
		methods: {
            ...
            getOrderByMonth() {
                const request = {
                    method: 'get',
                    url: '/api/report/order/groupBy/month',
                }
			...
			getOrderTotal() {
				const request = {
					method: 'get',
					url: '/api/report/order/total',
				}

				this.$store.dispatch('authRequest', request).then((response) => {
					this.dashboard.orderTotal = response;
				}).catch((error) => {
					this.$root.errorNotify(error)
				})
			},
			...
			getOrderSale() {
				const request = {
					method: 'get',
					url: '/api/report/order/sale',
				}

				this.$store.dispatch('authRequest', request).then((response) => {
					this.dashboard.orderSale = response;
				}).catch((error) => {
					this.$root.errorNotify(error)
				})
			},
			...
			getUserTotal() {
				const request = {
					method: 'get',
					url: '/api/report/user/total',
				}

				this.$store.dispatch('authRequest', request).then((response) => {
					this.dashboard.userTotal = response;
				}).catch((error) => {
					this.$root.errorNotify(error)
				})
			},
