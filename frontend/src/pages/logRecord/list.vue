    ...
    <section class="content" id="logRecord-list">
                    ...
                    <div class="card-header">
                        ...
                        <b-row class="justify-content-md-center">
                            ...
                            <b-col lg="2" md="6" sm="12" class="mt-3">
                                ...
                                <v-date-picker v-model="sentData.start_date" :model-config="dateConfig">
                            ...
                            <b-col lg="2" md="6" sm="12" class="mt-3">
                                ...
                                <v-date-picker v-model="sentData.end_date" :model-config="dateConfig">
                                    <template v-slot="{ inputValue, inputEvents }">
                                        <div class="flex items-center">
                                            <input type="text" class="form-control"
                                                   :value="inputValue"
                            ...
                            <b-col lg="2" md="6" sm="12" class="mt-3">
                                ...
                                <b-form-select v-model="sentData.log_id">
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover table-striped text-nowrap table-bordered">
                            ...
                            <tr>
                                ...
                                <th>{{ $t('logRecord.createdAt') }}</th>
                                <th>{{ $t('logRecord.operate') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="(data, index) in logRecordList" :key="index">
                                <td>{{data.log_record_id}}</td>
                                <td>{{ $t('logRecordSetting.'+ logRecordSetting[data.log_id].class) }}</td>
                                <td>{{data.account}}</td>
                                <td>
                                ...
                                <td>
                                    <a class="btn btn-primary" @click="view(data.log_record_id)"><i
                                            class="fas fa-eye"></i>
                                        {{ $t('common.view') }}</a>
                                </td>
                            </tr>
        ...
        <b-modal ref="logRecordEdit" cancel-title="取消" ok-title-html="確定" :title="$t('logRecord.' + method)" ok-only>
            <b-tabs content-class="mt-3" justified>
                    ...
                    <div class="d-block text-center">
                        <form class="form-horizontal">
                            <div class="card-body">
                                ...
                                <div class="form-group row text-left">
                                    <label for="remote_ip" class="col-sm-3">{{ $t('logRecord.remoteIp') }} : </label>
                                    <div class="col-sm-9">
                                        <span class="" id="remote_ip">{{ logRecord.remote_ip }}</span>
                                    </div>
                                </div>
                                <div class="form-group row text-left">
                                ...
                                <div class="form-group row text-left">
                                    <label for="new_date" class="col-sm-3"> {{ $t('logRecord.newDate') }} : </label>
                                    <div class="col-sm-9">
                                        <span class="" id="new_date">{{ logRecord.new_date }}</span>
                                    </div>
                                </div>
                                <div class="form-group row text-left">
                                    <label for="created_at" class="col-sm-3"> {{ $t('logRecord.createdAt') }}
                                        : </label>
                                    <div class="col-sm-9">
                                        <span class="" id="created_at">{{ logRecord.created_at }}</span>
                                    </div>
                                </div>
                            </div>
                        </form>
                ...
                <b-tab :title="$t('logRecord.content')">
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover table-striped text-nowrap table-bordered">
                            <thead>
                            <tr>
                                <th>{{ $t('logRecord.column') }}</th>
                                <th>{{ $t('logRecord.oldContent') }}</th>
                                <th>{{ $t('logRecord.newContent') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(data, index) in logRecord.new_content" :key="index">
                                    <td>{{index}}</td>
                                    <td>{{logRecord.old_content[index]}}</td>
                                    <td>{{data}}</td>
                                </tr>
                            </tbody>
                        </table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                sentData: {
                    ...
                    class: '',
                    log_id: '',
                    start_date: this.$dateTime.nowDate(),
                    end_date: this.$dateTime.tomorrow(),
                },
                //方法
                method: '',
                //是否loading
        ...
        methods: {
            ...
            getLogRecord() {
                ...
                if (this.sentData.log_id !== '') request.params.log_id = this.sentData.log_id
                if (this.sentData.start_date !== '') request.params.start_date = this.sentData.start_date
                if (this.sentData.end_date !== '') request.params.end_date = this.sentData.end_date
            ...
            getLogRecordSetting() {
                const request = {
                    method: 'get',
                    url: '/api/logRecord/setting',
                }

                this.$store.dispatch('authRequest', request).then((response) => {
                    this.logRecordSetting = response;
                }).catch((error) => {
	                this.$root.errorNotify(error)
                })
            },
            ...
            view(logRecordId) {
                ...
                this.$store.dispatch('authRequest', request).then((response) => {
                    ...
                    this.logRecord.name = this.logRecordSetting[this.logRecord.log_id]['class'];
