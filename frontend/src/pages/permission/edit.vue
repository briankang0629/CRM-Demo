                            ...
                            <form class="form-horizontal mt-3">
                                <div class="card-body">
                                    <div class="form-group row text-left">
                                        <label for="name" class="col-sm-3 col-form-label">{{ $t('permission.name') }}
                                            : </label>
                                        <div class="col-sm-9">
                                    ...
                                    <div class="form-group row text-left permission-setting">
                                        <label class="col-sm-3 col-form-label">{{ $t('permission.permission') }}
                                            : </label>
                                        <div class="col-sm-9">
                                            <div class="accordion" role="tablist">
                                                <!-- 權限 -->
                                                <b-card no-body class="mb-1" v-for="(data, menu) in permissionConfig"
                                                        :key="menu">
                                                    <b-card-header header-tag="header" class="p-1" role="tab">
                                                        <b-button block v-b-toggle="menu" variant="info">
                                                            {{ $t('menu.' + menu) }}
                                                        </b-button>
                                                    </b-card-header>
                                                    <b-collapse :id="menu" visible accordion="my-accordion"
                                                                role="tabpanel">
                                                        <!-- 有子選單 -->
                                                        <b-card-body v-show="typeof data === 'object'">
                                                        ...
                                                        <!-- 無子選單 -->
                                                        <b-card-body v-show="typeof data === 'string'">
                                                            <b-form-radio-group
                                                                v-model="permission.permission[menu]"
                                                                :options="status"
                                                                class="mb-3"
                                                                value-field="item"
                                                                text-field="name"
                                    ...
                                    <div class="form-group row text-left">
                                        ...
                                        <div class="col-sm-9">
                                            <b-form-select v-model="permission.status" :options="$t('status')"
                                                           :disabled="controlPermission !== 'E'">
                                                <template #first>
...
export default {
    ...
    mounted() {
        //permission
        this.controlPermission = this.$store.state.permission.admin.permission
    ...
    methods: {
        ...
        getPermissionConfig() {
            ...
            this.$store.dispatch('authRequest', request).then((response) => {
                this.permissionConfig = response;
        ...
        edit(permission_id) {
            ...
            const request = {
                method: 'get',
                url: '/api/permission/' + permission_id,
            }

            this.$store.dispatch('authRequest', request).then((response) => {
                this.permission = response
                this.permissionConfig = response.permission
                this.isLoading = false
            }).catch((error) => {
                this.$root.errorNotify(error)
                this.isLoading = false
        ...
        save() {
            ...
            const request = {
                ...
                url: '/api/permission' + (this.method === 'create' ? '' : '/' + this.permission.permission_id),
                data: this.qs.stringify(this.permission)
            }

            this.$store.dispatch('authRequest', request).then((response) => {
                this.$router.push({name: 'permission'}).then(() => {
        ...
        remove() {
            ...
            this.$Swal.fire({
                ...
                if (result.isConfirmed) {
                    let permission_id = this.selected.join(',')
                    //請求刪除
                    const request = {
                        method: 'delete',
                        url: '/api/permission/' + permission_id,
